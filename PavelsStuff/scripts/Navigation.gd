...
func _process(delta):
	_update_path()
	var test = get_closest_point(get_node("../player").get_translation())
	if (path.size() > 1):
...
func _update_path():
	begin = get_closest_point(get_node("../enem").get_translation())
	end = get_closest_point(get_node("../stop").get_translation())
