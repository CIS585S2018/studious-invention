...
func _ready():
	...
	for i in range(gridSize): #Generate the hexagon grid with uneven rows 
		...
		for j in range(rows): 
			...
			add_child(node)
			node.set_scale(Vector3(1.0/gridSize, 1.0 / 4, 1.0 / gridSize))
			node.set_translation(Vector3((offset + j) * (39.75 / gridSize), 0.55, ( ((gridSize - 1.0) / -2.0) + i) * (35.75 / gridSize)))
			var wall2 = node.get_node("Wall2")
			var wall3 = node.get_node("Wall3")
			var wall1 = node.get_node("Wall1")
			var wall4 = node.get_node("Wall4")
			var wall5 = node.get_node("Wall5")
			var wall6 = node.get_node("Wall6")
		...
		for j in range(rows):  
			if(i >= ((gridSize - 1.0) / 2.0)): #Checks if hex is in the bottom half of the map 
				if((j != 0) && (i != gridSize - 1)):  #If it is, should not add edges on outside of grid. 
					...
					edge.wall = hexList[i][j].get_node("Wall6")
					edge.nodeAY = i
					edge.nodeAX = j
					edge.nodeBY = i + 1
					edge.nodeBX = j - 1
				...
				if((j != (rows - 1.0)) && (i != gridSize - 1)): 
					var edge = hexEdge.instance()
					edge.wall = hexList[i][j].get_node("Wall5")
					edge.nodeAY = i
					edge.nodeAX = j
					edge.nodeBY = i + 1
					edge.nodeBX = j
				...
				edge2.wall = hexList[i][j].get_node("Wall5")
				edge2.nodeAY = i
				edge2.nodeAX = j
				edge2.nodeBY = i + 1
				edge2.nodeBX = j + 1
			...
			if(j != (rows - 1.0)): 
				var edge = hexEdge.instance()
				edge.wall = hexList[i][j].get_node("Wall4")
				edge.nodeAY = i
				edge.nodeAX = j
				edge.nodeBY = i
				edge.nodeBX = j + 1
